...
if ( file_exists( dirname(__FILE__) . '/vendor/autoload.php' ) ) {
	...
	if ( file_exists( dirname(__FILE__) . '/lessphp/lessc.inc.php' ) ) {
		// load LESS parser
...
if ( ! class_exists( 'wp_less' ) ) {
	...
	if (!function_exists('registerLess') && !function_exists('unregisterLess')){
		function registerLess($name, $callable){
			$less = wp_less::instance();
			$less->register($name, $callable);
		}
		function unregisterLess($name){
			$less = wp_less::instance();
			$less->unregister($name);
		}
	}

	if (!function_exists('addLessVar') && !function_exists('removeLessVar')){
		function addLessVar($name, $value) {
			$less = wp_less::instance();
			$less->add_var($name, $value);
		}
		function removeLessVar($name) {
			$less = wp_less::instance();
